...
def _get_logit_bias(processor: Optional[ProcessorMixin]) -> Optional[dict[int, float]]:
    # Prevent multimodal placeholder tokens from being generated in assistant text.
    ...
    for attr in ("image_token", "vision_start_token", "vision_end_token", "video_token"):
        ...
        if isinstance(tok, str):
            ...
            if isinstance(tok_id, int) and tok_id >= 0 and tok_id != tokenizer.unk_token_id:
                blocked_ids.add(tok_id)

    return {tok_id: -1e9 for tok_id in blocked_ids} if blocked_ids else None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        sampling_kwargs = {
            ...
            "detokenize": True if (config.multiturn and config.stop_strings) else False,
            "logit_bias": _get_logit_bias(processor),
            "stop": config.stop_strings.split(",") if (config.multiturn and config.stop_strings) else None,
            "include_stop_str_in_output": True if (config.multiturn and config.stop_strings) else False,
        }
